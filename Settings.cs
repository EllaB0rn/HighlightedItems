...
public class Settings : ISettings
{
    ...
    [Menu("Fast Mode", "Minimize all delays for maximum speed (may be less stable)")]
    public ToggleNode FastMode { get; set; } = new(false);

    [Menu("Retry Missed Items", "Retry moving items that were missed on first pass")]
    public ToggleNode RetryMissedItems { get; set; } = new(true);

    [Menu("Max Retry Attempts", "Maximum number of retry attempts for missed items")]
    public RangeNode<int> MaxRetryAttempts { get; set; } = new(2, 1, 5);
